...
const double yellowThreshold  = 20;
const double redThreshold     = 300;
const int inputDelay          = 300;
const byte calibParam         = 10;
...
double previousValues[prevValDim] = {0};
byte value;
...
void setup() 
{
  ...
  pinMode(LEDB,    OUTPUT);
  pinMode(LEDC,    OUTPUT);
  writeScreen("Doppler Radar", -1, "Breath Monitor", -1);
  delay(2000);
...
void calibrate()
{
  ...
  while(calibrateCount < calibParam)
    getValue(); 
...
void getValue()
{
  ...
  if(breath[1] > breath[0])
  {
    ...
    if(findAvg == true)
    {
      ...
      calibrateCount++;
      if(calibrateCount > calibParam + 1)
        calibrateCount = calibParam + 1;
    }   
  } 
}
      ...
      switch(option)
      {
        ...
        recentOption = 1;
          while(digitalRead(buttonB) == 0)
          {
            ...
            recentOption = checkLimits(recentOption, 1, prevValDim);
            writeScreen("Prev. Rate", recentOption, "", previousValues[recentOption - 1]);
          }
          delay(inputDelay);
          break;
        case 5:
